...
DEV_DF = pd.read_csv("./dev_set_wikihow.csv")
...
def evaluate(model, valid_loader, DEVICE): 
    ...
    with torch.no_grad(): 
        ...
        for (identification, pair_token_ids, mask_ids, seg_ids, fake_labels) in valid_loader: 
            ...
            seg_ids = seg_ids.to(DEVICE)
             # 16
            identification = identification.to(DEVICE)
            fake_labels = fake_labels.to(DEVICE)

            outputs_per_batch = model(input_ids, token_type_ids=seg_ids, attention_mask=attention_mask, labels=fake_labels)
           
            loss = outputs_per_batch.loss
            logits = outputs_per_batch.logits 
            print("logits", logits)
            # logits.max(1).indices
            outputs.append([identification, logits.max(1).indices])
    return outputs 

def encode_data(x_context, y_context): 
    ...
    for (x, y) in zip(x_context, y_context): 
        ...
        identifications.append(counter) 
    ...
    identifications = torch.tensor(identifications) 
    token_ids = pad_sequence(token_ids, batch_first=True)
    mask_ids = pad_sequence(mask_ids, batch_first=True)
    seg_ids = pad_sequence(seg_ids, batch_first=True)
    fake_labels = torch.tensor([0 for i in range(591)])
   
    dataset = TensorDataset(identifications, token_ids, mask_ids, seg_ids, fake_labels)
...
def main(): 
    ...
    x_context_dev, y_context_dev = read_entailment_data(DEV_DF)
    dev_encoded =  encode_data(x_context_dev, y_context_dev)
    ...
    model = torch.load("bertforentailment.pt")
    print("finished")
    ...
    for epoch, _ in enumerate(range(NUM_EPOCHS),1):
         print("------- epoch {0}".format(epoch))
         outputs = evaluate(model, dev_loader, DEVICE)
         print(outputs)
main()